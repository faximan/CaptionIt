...
{	
	...
	CGFloat red = pixelBuf[r];
	CGFloat green = pixelBuf[g];
	CGFloat blue = pixelBuf[b];
	
	CGFloat gray = 0.3f * red + 0.59f * green + 0.11f * blue;
	
	pixelBuf[r] = (Byte)gray;
	pixelBuf[g] = pixelBuf[r];
	pixelBuf[b] = pixelBuf[r];
}
...
{	
    ...
    CGFloat myred = (red * 0.393f) + (green * 0.769f) + (blue * 0.189f);
    pixelBuf[r] = (myred > 255.0f) ? 255 : (UInt8)myred;
    CGFloat mygreen = (red * 0.349f) + (green * 0.686f) + (blue * 0.168f);
    pixelBuf[g] = (mygreen > 255.0f) ? 255 : (UInt8)mygreen;
    CGFloat myblue = (red * 0.272f) + (green * 0.534f) + (blue * 0.131f);
    pixelBuf[b] = (myblue > 255.0f) ? 255 : (UInt8)myblue;
}
...
void filterSaturate(UInt8 *pixelBuf, UInt32 offset, void *context)
{
	...
	CGFloat avg = ( red + green + blue ) / 3;
	CGFloat newred = avg + t * (red - avg);
    CGFloat newgreen = avg + t * (green - avg);
    CGFloat newblue = avg + t * (blue - avg);
    
    pixelBuf[r] = (newred > 255.0f) ? 255 : (Byte)newred;
    pixelBuf[g] = (newgreen > 255.0f) ? 255 : (Byte)newgreen;
    pixelBuf[b] = (newblue > 255.0f) ? 255 : (Byte)newblue;
